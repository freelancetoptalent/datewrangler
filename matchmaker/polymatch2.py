...
from register.models import RecessRecord
...
class SeatableDater(Dater):
    '''This is a dater, but with added information for us to put them at
       tables (.is_group and .is_stationary).'''
    def __init__(self, name, is_group, is_stationary, d=None):
        self.is_group = is_group
        self.is_stationary = is_stationary
        super(SeatableDater, self).__init__(name, d)
...
class TableSet():
    ...
    def claim_table(self, stat=False, group=False):
        if stat and group:
            sources = [self.av_gs]
        elif stat:
            sources = [self.av_s, self.av_gs]
        elif group:
            sources = [self.av_g, self.av_gs]
        else:
            sources = [self.av_x, self.av_g, self.av_s, self.av_gs]
        for source in sources:
            if not source:
                continue
            table = source.pop()
            self.unavailable.add(table)
            return table
        raise ValueError("No tables available for spec stationary=%s, group=%s" % (stat, group))

    def reset(self):
        self.load(self.unavailable)


def check_table_acceptability(daters, tables):
    n_stationary = len([d for d in daters if d.is_stationary])
    n_stat_g = len([d for d in daters if d.is_stationary and
                 (d.is_group or any(id.endswith('G') for id in d))])
    n_groups = len([d for d in daters if d.is_group])
    n_nonstat_groups = len([d for d in daters if d.is_group and not d.is_stationary])

    n_group_tables = len([t for t in tables if t.group_ok])
    n_stationary_tables = len([t for t in tables if t.stationary_ok])
    n_both_tables = len([t for t in tables if t.group_ok and t.stationary.ok])

    return n_both_tables >= n_stat_g
        and n_stationary_tables >= n_stationary
        and n_group_tables - n_both_tables >= n_nonstat_groups
    


def go(daters, n_rounds, what):
    ## initialize
    dater_d = dict((d.name, d) for d in daters)
    schedule_d = dict((d.name, Itinerary()) for d in daters)
    tables_d = {}
    for i in range(n_rounds):
        tableset = TableSet()
        tableset.load(tables)
        tables_d[i+1] = tableset
    ...
    upcoming = []
    active = [x[1] for x in sorted((-len(d), d) for d in daters)]

    ## main loop
    while upcoming:
        while len(active) > 0:
            user = active.pop()
            if len(schedule_d[user.name]) >= n_rounds:
                out.append(user)
                continue

            tried = set()
            while True:
                # grand partner-picking loop
                while True:
                    partner = pick_next_date(user, schedule_d[user.name], tried)
                    if partner is None or is_currently_available(schedule_d[partner.name], loop):
                        break
                    tried.add(partner)
                if partner is None:
                    # pick_next_date must have said we're totally out of options at the moment
                    # otherwise, we have someone we can at least try
                    break

                rounds = mutual_rounds_free(schedule_d[user.name], schedule_d[partner.name])
                need_stat = user.is_stationary or partner.is_stationary
                need_group = user.is_group or partner.is_group
                table = None
                for round in rounds:
                    try:
                        table = tables_d[round].claim_table(need_stat, need_group)
                    except ValueError:
                        # no such table this round
                        continue
                if table is None:
                    # then NO round worked
                    # so, not this person.
                    # since tables never free up as we go, they are no longer a possible date this event
                    del user[partner.name]
                    continue
                # if we get here, we have a valid table/round/partner combo!
                match_type = 'similar' if is_this_gay(user, partner) else 'different'
                schedule_d[user.name][round] = (partner.name, match_type)
                schedule_d[partner.name][round] = (user.name, match_type)
                del user[partner.name]
                del partner[user.name]
                if len(schedule_d[user.name]) >= n_rounds:
                    out.append(user)
                else:
                    upcoming.append(user)
    
        active = [x[1] for x in sorted((-len(d), d) for d in upcoming)]
        upcoming = []
        while active and len(active[-1]) == 0:
            out.append(active.pop)
        if not active:
            # then everybody is out of possible dates
...
def pick_next_date(possible, existing, excludes):
    # this is where we decide who a person would date next given their druthers
    # that person could still be unsuitable for contingent reasons
    pass
        
def is_currently_available(person, loop_n):
    x = 1
    # this is where we rule out people who are currently ahead of the curve for date cont
    pass

def mutual_rounds_free(a, b):
    # this is where we list rounds that are free in two schedules
    pass
...
def make_schedules_random(event_name, daters, rounds, tables, trials=20, scramble_rounds=True):
    
    return make_schedules_random_inner(event_name, daters, rounds, trials=20)

def make_schedules_random_inner(event_name, daters, rounds, trials=20, scramble_rounds=True):
    best = None
    best_score = 1000000
    for i in range(trials):
